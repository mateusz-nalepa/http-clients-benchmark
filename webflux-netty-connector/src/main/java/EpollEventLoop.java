...
//                        System.out.println("BUSY_WAIT:" + Thread.currentThread().getName());
                        break;
...
//                        System.out.println("SELECT:" + Thread.currentThread().getName());
    ...
    private boolean processReady(EpollEventArray events, int ready) {
        ...
        var dummyClientDuration = Duration.ofNanos(end - start);
        Metrics
                .timer("EpollEventLoop_processReady",
                "thread", Thread.currentThread().getName(),
                        "eventsSize", String.valueOf(ready)
                ).record(dummyClientDuration);

        if (ready != 0) {
            var xd = (end - start) / ready;
            Metrics.timer("EpollEventLoop_IleSrednioJeden").record(Duration.ofNanos(xd));
        }
        return timerFired;
    }
