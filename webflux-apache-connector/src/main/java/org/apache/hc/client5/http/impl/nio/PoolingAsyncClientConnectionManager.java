...
//        NALEPA_LOG.error("{} Pooling async connection manager lease", Thread.currentThread().getName());
        if (LOG.isDebugEnabled()) {
...
//                                        NALEPA_LOG.error("{} Manager robie dicardConnection: ", Thread.currentThread());
                ...
                new FutureCallback<ManagedAsyncClientConnection>() {
                    ...
                    public void completed(final ManagedAsyncClientConnection connection) {
                        try {
                            ...
                            Duration connectionDuration = Duration.ofNanos(endConnectTime - startConnectTime);
//                            NALEPA_LOG.error("{} Manager connect took: {}", Thread.currentThread(), connectionDuration);
                            Metrics.timer("managerConnect").record(connectionDuration);
                        } catch (final RuntimeException ex) {
                            resultFuture.failed(ex);
                        }
                    }
