    ...
    final boolean checkTimeout(final long currentTimeMillis) {
        ...
        if (!timeout.isDisabled()) {
            ...
            if (currentTimeMillis > deadlineMillis) {
                try {
                    // HODOR - tutaj jest to 511?, w sensie ten timeoutMillis
                    Long startDiscardConnection = System.nanoTime();
                    onTimeout(timeout);
                    Long endDiscardConnection = System.nanoTime();
                    Duration dicardConnectionDuration = Duration.ofNanos(endDiscardConnection - startDiscardConnection);
                    Metrics.timer("discardConnection").record(dicardConnectionDuration);
                } catch (final CancelledKeyException ex) {
                    close(CloseMode.GRACEFUL);
                } catch (final Exception ex) {
                    onException(ex);
